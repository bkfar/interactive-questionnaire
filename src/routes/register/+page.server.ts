...
export const actions: Actions = {
  default: async (event: RequestEvent) => {
    ...
    if (!email || !password || !username) {
      return fail(400, { error: 'All fields are required.', values: { email, username } });
    }
    ...
    if (user) {
      ...
      if (profileError) {
        return fail(500, { error: 'Registration succeeded, but failed to set username.', values: { email, username } });
      }
    }
